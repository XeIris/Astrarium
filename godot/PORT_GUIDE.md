...
This is the engineering contract for the Godot 4.7 port that lives in `godot/`.
The web build (repo root) is the **reference implementation**, and it stays
live and shippable. Read the repo's `CLAUDE.md` first. Its conventions are the
...
* **Every renderer camera sits at the origin (rotation only).** Each frame the
  ...
  (CLAUDE.md, "a body's vertex shader never forms an absolute world
...
| `renderOrder` | `material.render_priority` (it sorts within opaque/transparent lists only, as three's did; CLAUDE.md) |
...
inner nodes. This is the CLAUDE.md rule "a stage builder must not write to
...
* **Harnesses.** `tools/harness.gd` is a base class that brings up the real
  ...
  `/Applications/Godot.app/Contents/MacOS/Godot --path godot res://tools/x.tscn -- out=/abs/x.png frames=30`
  (the process quits by itself after writing the file; run it in the background
  and wait, because a window opens).
* **Reference frames from the web build.** `node godot/tools/webref.mjs shots.json outdir/`
...
* **Numbers before pictures.** Pure modules (physics, structure, flight) are
  ...
  (`Godot --headless --path godot --script res://tools/x.gd`), and compare.
* **Script errors.** `Godot --headless --path godot --import` then
  `Godot --headless --path godot --quit` surfaces parse errors. Shader compile
