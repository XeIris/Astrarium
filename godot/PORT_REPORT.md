...
`godot/`. It lives on branch `godot-port`, about 100 commits on top of `main`. The web
build at the repo root is untouched and remains the reference.
...
- **Body derivation** (`deriveBody`, `renderRadius`, the type tables) moved from
  the orchestrator into `sim/derive.gd`, as CLAUDE.md prefers.
...
## 2. Verification
The standing checks from CLAUDE.md, plus a side-by-side comparison of every
...
| Interior model, presets, climate | 1,048 `structure_of` cases and 48,918 numbers: max relative error 2.8e-14; every CLAUDE.md calibration reproduced |
...
The evidence behind each row is in `godot/tools/ref/` (web and Godot image pairs,
plus state dumps).
...
- `godot/tools/ref/` is **80 MB** of comparison images. They are the evidence
  for this report but don't need to live in the repo. Deleting that folder
  changes nothing about running or building.
- The parallel porting work used git worktrees under `.claude/worktrees/`, one
