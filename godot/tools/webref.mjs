...
//           the page has a SIM
//   dump    JS expression (may be async) evaluated after the capture; its
//           JSON value is written to outdir/<name>.json
//
...
for (const s of shots) {
  ...
  try {
    ...
    if (s.dump) await writeFile(join(outDir, s.name + '.json'), JSON.stringify(await evaluate(`(async () => (${s.dump}))()`), null, 1));
