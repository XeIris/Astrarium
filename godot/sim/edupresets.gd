...
#
# What makes one of these different from a scenario in sim/presets.gd is what
...
static func _make() -> Dictionary:
	...
	# its own contraction — see PHASES in sim/structure.gd — and it is bigger,
	...
	P.edu_starbirth = {
		...
		# silhouette. sim/sky.gd already has H II regions, reflection nebulosity
	...
	P.edu_lifecycle = {
		...
		"build": func() -> Array:
			...
			return [
				...
				# star, and _star_structure() then holds them fixed against everything
	...
	# that rather than assuming it — see sim/lightcurve.gd — which is why it
	...
	# (insolation_at() in sim/suns.gd), turns that into a surface temperature, and
	...
	#
	# Everything in sim/sky.gd is band-aware, which is what makes the multi-
