...
// sim/lessonui.js executes against a small stage API handed to it by the
...
//
// THE `do` VOCABULARY
//   preset      scenario key (see sim/presets.js and sim/edupresets.js)
//   focus       body name to follow
//   cam         { radius, theta, phi, mode }  — mode: orbit | free | surface
//   band        imaging band index, 0–6 (see sim/spectrum.js BANDS)
...
const svg = (viewBox, inner) =>
  `<svg viewBox="${viewBox}" class="lfig" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">${inner}</svg>`;
...
export const FIGURES = {
  parallax: svg('0 0 320 150', `
    ...
    <text x="140" y="78" fill="#ffd28a" font-size="9" text-anchor="end">p</text>`),

  ladder: svg('0 0 320 128', `
    ${[['radar', 'the solar system', 10, '10⁻⁴'],
       ['parallax', 'to ~10 000 ly', 72, '10⁴'],
       ['main-seq fitting', 'clusters', 134, '10⁵'],
       ['Cepheids', 'nearby galaxies', 196, '10⁸'],
       ['Type Ia', 'the far universe', 258, '10¹⁰']]
      .map(([a, b, x, ly]) => `
        <rect x="${x}" y="34" width="54" height="40" fill="none" stroke="currentColor" stroke-opacity=".4"/>
        <text x="${+x + 27}" y="50" text-anchor="middle" fill="#ffd28a" font-size="8">${a}</text>
        <text x="${+x + 27}" y="62" text-anchor="middle" fill="currentColor" font-size="7.5" opacity=".75">${b}</text>
        <text x="${+x + 27}" y="88" text-anchor="middle" fill="#7fc4ff" font-size="8">${ly} ly</text>`).join('')}
    ...
    <text x="160" y="112" text-anchor="middle" fill="currentColor" font-size="8" opacity=".7">an error low down propagates all the way up</text>`),

  redshift: svg('0 0 320 140', `
    <text x="160" y="14" text-anchor="middle" fill="currentColor" font-size="9">the same spectrum, three distances</text>
    ${[[30, 0, 'nearby'], [66, 26, '×2 further'], [102, 52, '×4 further']].map(([y, sh, lab]) => `
      <rect x="40" y="${y}" width="240" height="18" fill="url(#g${y})"/>
      <line x1="${92 + sh}" y1="${y}" x2="${92 + sh}" y2="${+y + 18}" stroke="#000" stroke-width="2"/>
      <line x1="${128 + sh}" y1="${y}" x2="${128 + sh}" y2="${+y + 18}" stroke="#000" stroke-width="2"/>
      <text x="34" y="${+y + 13}" text-anchor="end" fill="currentColor" font-size="8">${lab}</text>`).join('')}
    <defs>${[30, 66, 102].map(y => `
      <linearGradient id="g${y}" x1="0" x2="1">
    ...
    <text x="160" y="134" text-anchor="middle" fill="currentColor" font-size="8" opacity=".75">
      the lines shift toward the red in proportion to distance — v = H₀d</text>`),

  binding: svg('0 0 320 150', `
    ...
    <text x="16" y="72" fill="currentColor" font-size="8" transform="rotate(-90 16 72)" text-anchor="middle">binding energy / nucleon</text>`),

  emspectrum: svg('0 0 320 92', `
    ...
    ${[['radio', 24], ['micro', 70], ['IR', 112], ['vis', 160], ['UV', 200], ['X', 246], ['γ', 292]]
      .map(([n, x]) => `<text x="${x}" y="20" text-anchor="middle" fill="currentColor" font-size="8">${n}</text>
        <line x1="${x}" y1="22" x2="${x}" y2="26" stroke="currentColor" stroke-opacity=".5"/>`).join('')}
    ...
    <text x="160" y="80" text-anchor="middle" fill="currentColor" font-size="8" opacity=".75">
      the visible band is the narrow strip our eyes evolved for — not a special part of physics</text>`),

  transitgeom: svg('0 0 320 116', `
    ...
    <text x="300" y="36" text-anchor="end" fill="currentColor" font-size="7.5" opacity=".7">depth</text>`),
};
...
// ---------------------------------------------------------------------------
{
  id: 'sky', title: 'The sky from here', icon: '☉',
  ...
  { id: 'scale', title: 'How big, how far', mins: 4,
  ...
  { id: 'daynight', title: 'Why the sky turns', mins: 4,
    goal: 'Separate what the Earth does from what the sky appears to do.',
    steps: [
      { title: 'Stand on the planet',
        do: { preset: 'edu_seasons', focus: 'Earth', cam: { mode: 'surface' }, timeScale: 0.003, localTime: 'morning' },
  ...
  { id: 'seasons', title: 'Why there are seasons', mins: 6,
    ...
    steps: [
      ...
      { title: 'The two hemispheres disagree',
        do: { preset: 'edu_seasons', cam: { mode: 'surface' }, timeScale: 0.03, control: { lat: -35 }, localTime: 'noon' },
  ...
  { id: 'phases', title: 'Phases of the Moon', mins: 5,
  ...
  { id: 'eclipses', title: 'Eclipses, and why they are rare', mins: 4,
...
// ---------------------------------------------------------------------------
{
  id: 'gravity', title: 'Gravity and orbits', icon: '◍',
  ...
  { id: 'ellipse', title: 'Orbits are ellipses', mins: 5,
  ...
  { id: 'harmonic', title: 'The law that weighs the universe', mins: 5,
  ...
  { id: 'nbody', title: 'When there are three', mins: 4,
  ...
  { id: 'tides', title: 'Tides, and how to tear a moon apart', mins: 4,
...
// ---------------------------------------------------------------------------
{
  id: 'light', title: 'Light, the only messenger', icon: '≈',
  ...
  { id: 'inverse', title: 'Brightness and distance', mins: 4,
  ...
  { id: 'blackbody', title: 'Colour is temperature', mins: 5,
    ...
    steps: [
      ...
      { title: 'The classes are a temperature sequence',
        do: { preset: 'hr_ladder', cam: { radius: 40 } }, instrument: 'hr',
  ...
  { id: 'spectrum', title: 'The seven skies', mins: 6,
...
// ---------------------------------------------------------------------------
{
  id: 'stars', title: 'The Sun, and the other stars', icon: '★',
  ...
  { id: 'sun', title: 'The Sun is a star', mins: 6,
    goal: 'What a star is, all the way down.',
    steps: [
      { title: 'A ball of gas holding itself up',
        do: { preset: 'edu_sun', focus: 'Sun', cam: { radius: 6 }, timeScale: 0.0028, panel: { coursePanel: false, xsecPanel: true } },
        instrument: 'cutaway',
  ...
  { id: 'activity', title: 'A star with weather', mins: 6,
  ...
  { id: 'distances', title: 'How far away is a star?', mins: 5,
  ...
  { id: 'hr', title: 'The most important diagram in astronomy', mins: 6,
    goal: 'The HR diagram, and what it means that stars are not scattered.',
    steps: [
      { title: 'Plot them and see',
        do: { preset: 'hr_ladder', cam: { radius: 46 } }, instrument: 'hr',
      ...
      { title: 'The clumps off the band',
        do: { preset: 'stellar_zoo', trueScale: true, cam: { radius: 60 } }, instrument: 'hr',
  ...
  { id: 'binaries', title: 'How to weigh a star', mins: 5,
...
// ---------------------------------------------------------------------------
{
  id: 'lives', title: 'The lives of stars', icon: '✦',
  ...
  { id: 'birth', title: 'Where stars come from', mins: 5,
    ...
    steps: [
      ...
      { title: 'Not a star yet',
        do: { preset: 'edu_starbirth', focus: 'Protostar', cam: { radius: 14 }, panel: { coursePanel: false, xsecPanel: true } },
        instrument: 'cutaway',
  ...
  { id: 'mainseq', title: 'Mass is destiny', mins: 5,
  ...
  { id: 'giants', title: 'Leaving the main sequence', mins: 6,
    ...
    steps: [
      ...
      { title: 'The core runs out, and the star gets bigger',
        do: { preset: 'edu_lifecycle', panel: { coursePanel: false } }, instrument: 'cutaway',
  ...
  { id: 'supernova', title: 'The death of a heavy star', mins: 6,
    goal: 'Core collapse, and what is left.',
    steps: [
      { title: 'The onion',
        do: { preset: 'edu_supernova', focus: 'Doomed', cam: { radius: 44 }, panel: { coursePanel: false, xsecPanel: true } },
        instrument: 'cutaway',
  ...
  { id: 'neutron', title: 'Neutron stars and pulsars', mins: 5,
...
// ---------------------------------------------------------------------------
{
  id: 'holes', title: 'Black holes and spacetime', icon: '●',
  ...
  { id: 'escape', title: 'Escape velocity, taken to its limit', mins: 5,
  ...
  { id: 'shadow', title: 'The shadow and the ring', mins: 5,
  ...
  { id: 'accretion', title: 'How black holes shine', mins: 4,
  ...
  { id: 'gw', title: 'Ripples in spacetime', mins: 6,
    goal: 'What a gravitational wave is, and what it does to a detector.',
    steps: [
      { title: 'Two holes, spiralling in',
        do: { preset: 'bhmerger', cam: { radius: 72 }, timeScale: 0.08 }, instrument: 'gw',
      ...
      { title: 'And when neutron stars do it',
        do: { preset: 'nsmerger', cam: { radius: 26 }, timeScale: 0.08 }, instrument: 'gw',
...
// ---------------------------------------------------------------------------
{
  id: 'galaxies', title: 'Galaxies and the universe', icon: '◈',
  ...
  { id: 'milkyway', title: 'The galaxy we are inside', mins: 5,
  ...
  { id: 'clusters', title: 'Star clusters', mins: 4,
  ...
  { id: 'galaxytypes', title: 'A hundred billion others', mins: 5,
    ...
    steps: [
      ...
      { title: 'Spirals, ellipticals and the mess',
        do: { preset: 'edu_galaxy', sky: { env: ['disc', 'starburst'] }, cam: { radius: 12 } },
  ...
  { id: 'bigbang', title: 'The expanding universe', mins: 5,
...
// ---------------------------------------------------------------------------
{
  id: 'worlds', title: 'Other worlds', icon: '◉',
  ...
  { id: 'planets', title: 'What a planet looks like, and why', mins: 5,
  ...
  { id: 'transit', title: 'Finding planets: the transit', mins: 6,
    ...
    steps: [
      ...
      { title: 'What else a transit tells you',
        do: { preset: 'edu_transit', cam: { radius: 20, theta: 1.5708 } }, instrument: 'photometer',
  ...
  { id: 'wobble', title: 'Finding planets: the wobble', mins: 5,
    ...
    steps: [
      ...
      { title: 'It is a speed measurement, in a spectrum',
        do: { preset: 'edu_transit', cam: { radius: 10, theta: 1.5708 } }, instrument: 'photometer',
      ...
      { title: 'The two methods together',
        do: { preset: 'edu_transit', cam: { radius: 26, theta: 1.5708 } }, instrument: 'photometer',
  ...
  { id: 'habitable', title: 'The habitable zone', mins: 5,
  ...
  { id: 'alone', title: 'Are we alone?', mins: 4,
...
// ---------------------------------------------------------------------------
export const LESSON_ORDER = MODULES.flatMap(m =>
  m.lessons.map(l => ({ moduleId: m.id, lessonId: l.id, key: `${m.id}/${l.id}` })));

export const LESSON_COUNT = LESSON_ORDER.length;
export const STEP_COUNT = MODULES.reduce((a, m) =>
  a + m.lessons.reduce((b, l) => b + l.steps.length, 0), 0);

export function findLesson(key) {
  const [mid, lid] = String(key || '').split('/');
  const mod = MODULES.find(m => m.id === mid);
  const lesson = mod?.lessons.find(l => l.id === lid);
  return lesson ? { module: mod, lesson, key } : null;
}

export function neighbours(key) {
  const i = LESSON_ORDER.findIndex(e => e.key === key);
  return {
    index: i,
    prev: i > 0 ? LESSON_ORDER[i - 1].key : null,
    next: i >= 0 && i < LESSON_ORDER.length - 1 ? LESSON_ORDER[i + 1].key : null,
  };
}
...
// runner here; this is the next best thing, and sim/lessonui.js runs it once.
export function presetsUsed() {
  const set = new Set();
  for (const m of MODULES) for (const l of m.lessons) for (const s of l.steps) {
    if (s.do?.preset) set.add(s.do.preset);
  }
  return [...set];
}
