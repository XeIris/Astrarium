> Historical port plan. The completed Godot project is now at the repository root; this web version is archived in `web/`.
...
1. Read `../AGENTS.md` first — it is the engineering map and its conventions
...
4. **Beware two traps that each cost a debugging round trip:** GLSL reserves
   ...
   terminates the string. Both are documented in `../AGENTS.md`; both were hit
